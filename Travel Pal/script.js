onload = function () {
    ...
    const cities = ['Dhanbad', 'Meerut', 'Gujarat', 'Goa', 'Kanpur', 'Jammu', 'Hyderabad', 'Bangalore', 'Gangtok', 'Delhi'];
    ...
    genNew.onclick = function () {
        ...
        temptext2.innerText = 'FIND LEAST WEIGHT FROM '+cities[src-1]+' TO '+cities[dst-1];
        temptext.style.display = "inline";
        temptext2.style.display = "inline";
        temptext2.style.fontWeight="bold";
    ...
    function shouldTakebus(edges, dist1, dist2, mn_dist) {
        let bus = 0;
        let p1=-1, p2=-1;
    ...
    function solveData() {
        ...
        let {bus, p1, p2} = shouldTakebus(data['edges'], dist1, dist2, mn_dist);

        let new_edges = [];
        if(bus!==0){
            new_edges.push({arrows: { to: { enabled: true}}, from: p1+1, to: p2+1, color: 'green',label: String(bus)});
